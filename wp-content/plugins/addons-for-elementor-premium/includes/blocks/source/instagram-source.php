...
class LAE_Instagram_Grid_Source extends LAE_Source {
    ...
    public function get_media() {
        ...
        if (isset($this->item['image']) && !empty($this->item['image']['url'])):
            ...
            $image_html = sprintf('<img data-no-lazy="1" src="%s" title="%s" alt="%s" class="lae-image %s"/>', esc_url($this->item['image']['url']), esc_html($this->item['image']['alt']), esc_html($this->item['image']['alt']), lae_disable_lazy_load_classes());
