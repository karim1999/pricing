...
if( $arprice_class && $arp_chk_version ){
	$setvaltolic = $arprice_class->$arp_chk_version();
}
...
if(1 != $setvaltolic){
	$arprice_active_text = "<font color='#ff0000'>".$arprice_active."</font>";
} else {
	$arprice_active_text = "<font color='#10bd0d'>".$arprice_active."</font>";
	if( get_option('arpSortInfo') ){
	    ...
	    $arprice_license_details .= '<font color="white">'.$pcodeinfo.' | '.$pcodedate.' | '.$pcodedateexp.' | '.$pcodecustemail. '</font>';
	}
}
